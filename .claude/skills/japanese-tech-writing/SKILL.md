...
description: 日本語の技術文書・書籍原稿の文章規範。整形（一文一行、引用ブロック、脚注、コラム記法）、段落と論証の構成（パラグラフライティング）、論証の厳密さ（ツッコミどころの除去）、読み手の負荷の管理、視点と語り、演出の抑制、LLM っぽい空句の禁止、冗長の排除を定める。日本語で技術書の章や草稿を書くとき、または推敲・リライトするときに使用する。nextb.net のコラム記事本文には使わない（`column-writing` が正本。2026-08-02 ユーザー決定）。
---
...
- 裏付けのない断定を見つけたら、緩めるだけで終えない。足すべきものは2種類あり、混同しない。
    - 主張そのものを支える**根拠**（観察した事実、出典、本文内で確定した命題）
    - 因果を主張しているなら、なぜそうなるのかの**機構**。機構は因果のもっともらしさを示すだけで、事実の裏付けにはならない
    - 「A は B を大きく左右する」を「A は B に効いてくる」に直しても、根拠も機構もなければ主張は宙に浮いたままである。緩和は、根拠の不在を語調で隠すだけで解決しない。
    - 緩めた結果その段落が何も主張しなくなるなら、緩めるのではなく根拠を足すのが正しい。示せないなら、その主張ごと落とす。
    - これは不確実性を消せという意味ではない。推量・可能性・読者の疑念として書かれた文は、根拠を足したうえで不確実性を保つ（前掲の規則が優先する）。
- 推敲で文をつないだ箇所は、つないだ直後に読み直す。分割されていた文を結合すると、次の欠陥が生じやすい。
...
- 専門用語は、初出の文で説明する（本規範は一文一行なので、初出の行と一致する）。名前を出してから数行あと、まして別の節で説明するのでは遅い。
    - 順序は問わない。「用語（語釈）」の同格挿入は定義と同時の導入とみなし、前掲の「節の中心となる語は定義してから使う」を満たす。
    - まず、その語が本当に要るかを見る。後で参照しない語は、説明を足すのではなく語ごと削る（前掲の省略規則が優先する）。
    - 残った語のうち、想定読者が知らない可能性のあるものにだけ、丸括弧か同格で短い語釈を付ける（「**管理策**（リスクを下げるために実施する具体的な対策）」）。既知の語に語釈を足さない。
    - 名前を挙げただけで、それが何をするものかを最後まで書いていない語を残さない。読者にとっては、説明のない語は存在しないのと同じである。
    - 語釈を足しすぎると文が重くなる。1文に語釈は1つまでとし、2つ以上要るなら文を分ける。
- 提示前に、専門用語の初出を機械的に洗い出して確認する。想定読者を明示して読み返すだけでは、書き手には既知の語が素通りする。
    - 手順は、本文から専門用語を拾って一覧にし、各語の初出行を検索して、その行に語釈があるかを1件ずつ見る。読み返しで代用しない。
