...
description: 説明的な文章に緩急を設計するための規範。緩急を装飾ではなく認知モードの切替（観察→逡巡→断定→再観察）と未回収の緊張の管理として扱い、文の拍、段落の密度波形、節の入り方、緩みと駄文の判別、執筆後の機械的な点検手順を定める。読み物として最後まで読ませたい書籍の章・エッセイを生成するとき、または「密度はあるが平坦でおもしろくない」文章を診断・修正するときに使用する。nextb.net のコラム記事本文には使わない（`column-writing` が正本。読者が調べ物として読みに来るコラムでは、未回収の緊張を維持する本規範の中核が目的と衝突する。2026-08-02 ユーザー決定）。
---
...
- 拍は文長の分布で作る。短文が3つ以上続いたら、それは拍ではなく平坦化である。
  - 短文・長文は文字数で判定する。目安は30字以下が短文、55字超が長文、その中間が中位。
...
- 波形は段落のあいだだけでなく、節と節のあいだにも要る。同じ型の節を3つ以上続けない。
  - 「導入の一文＋ラベル付き項目の列挙＋一般論の締め」を繰り返すと、読者は2節目で型を学習し、以降を読まずに済ませられる。密度があるのに読まれない文章は、たいていこれが原因である。
  - 全節を別々の型にする必要はない。禁じるのは同型の連続と、文書の大半が同型になることである。
  - 崩す手は2つある。節の入り方（反問・告白・場面・断定）を変えるか、項目の並べ方（列挙・対比・時間順）を変える。とくに「まず／次に／最後に」の数え上げを全節で使わない。
...
- 結びの節を、冒頭の言い換えにしない。冒頭で述べた主張を抽象語で並べ直しただけの節は、回収ではなく重複である。
  - 前項の「冒頭の具体へ着地させる」とは別物である。両者は、その節に冒頭時点では書けなかった内容があるかで見分ける。あれば回収、なければ重複。
  - 判定は「この節を削って、読者が失うものがあるか」。失うものがないなら、削るか、読者がその時点で判断できる材料（到達点の確認、次の分岐、残っている宿題）に作り替える。
...
2. **漏出テスト**：この規範の語彙・例文（「答えの半分」「緊張」「回収」「線を引く」「問いを〜返す」等）が本文にそのまま現れていないか検索する。現れていれば、装置を宣言してしまった証拠なので、その文を削除し、装置を内容の側で実現し直す（ただし、例外2で許可された境界の回収文はこの削除対象に含めない）。あわせて、節末に「次は〜」型の進行予告がないかを全節末で確認する。
3. **緊張台帳**：本文中で立てた問い・思い込み・約束（「答えは半分ずつ返す」等）を列挙し、それぞれの回収位置を行で指す。指せないものは、回収を書き足すか、問いごと削る。
4. **拍の点検**：長い断定文が3つ以上連続している箇所を探し、短い足場か停止、または逡巡を挿む。短文が3つ以上連続している箇所も同じように探し、つながる内容をまとめて、短文は止めたい位置だけに残す。感覚で判断せず、文字数を数える（30字以下が短文、55字超が長文）。
5. **境界の点検**：二人称の呼びかけ・依頼・謙抑が本文中盤にないか確認する。あれば境界へ移すか削る。
6. **同型の点検**：節の見出し、各節の最初の段落、各節の項目の並べ方を抜き出して並べる。同じ型が3つ以上続いていたら、入り方か並べ方のどちらかを変える。
7. **結合部の点検**：推敲で文をつないだ箇所を読み直す。生じやすい欠陥の一覧は `../japanese-tech-writing/SKILL.md` の「論証の厳密さ」に置く。
...
- **文が細かく刻まれて単調**：拍を作るつもりの分割が積み重なっている。点検手順4で文字数を数え、つながる内容をまとめて、短文は止めたい位置だけに残す。
- **節がどれも同じ形に見える**：節間の波形がない。点検手順6を適用する。
